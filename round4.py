...
class MacaronStrategy(Strategy):
    def __init__(self,
                 symbol: str = "MAGNIFICENT_MACARONS",
                 limit: int = 75,
                 sun_window_size: int = 100,
                 below_count: int = 10,
                 quantile: float = 0.4):
        super().__init__(symbol, limit)
        self.sun_window = deque(maxlen=sun_window_size)
        self.below_count = below_count
        self.quantile = quantile

    def act(self, state: TradingState) -> list[Order]:
        orders: list[Order] = []
        # Fetch latest conversion observation
        obs = state.observations.conversionObservations.get(self.symbol)
        if not obs:
            self.traderData = "no observation"
            return orders
        sunlight = obs.sunlightIndex
        # Append to sunlight history
        self.sun_window.append(sunlight)

        # Need full window and valid book to trade
        depth = state.order_depths.get(self.symbol)
        if len(self.sun_window) < self.sun_window.maxlen or not depth or not depth.buy_orders or not depth.sell_orders:
            self.traderData = f"warming up: {len(self.sun_window)}/{self.sun_window.maxlen}"
            return orders

        # Compute CSI (20th percentile)
        csi = np.percentile(list(self.sun_window), self.quantile * 100)
        # Check if sunlight has been below CSI for the last BELOW_COUNT ticks
        recent = list(self.sun_window)[-self.below_count:]
        sustained_below = len(recent) == self.below_count and all(s < csi for s in recent)

        # Mid-price for execution
        best_bid = max(depth.buy_orders.keys())
        best_ask = min(depth.sell_orders.keys())
        mid_price = (best_bid + best_ask) / 2
        position = state.position.get(self.symbol, 0)

        # Entry: sustained low sunlight → full long
        if sustained_below and position < self.limit:
            size = self.limit - position
            orders.append(Order(self.symbol, mid_price, size))
            action = f"entry size={size}"
        # Exit: sunlight recovers above CSI → unwind
        elif sunlight > csi and position > 0:
            orders.append(Order(self.symbol, mid_price, -position))
            action = f"exit size={-position}"
        else:
            action = "hold"

        # Attach debugging info
        self.traderData = (
            f"sunlight={sunlight:.1f}, CSI={csi:.1f}, pos={position}, action={action}"
        )
        return orders
    ...
    def run(self, state: TradingState):
        ...
        for symbol, strategy in self.strategies.items():
            if symbol in state.order_depths:
                ...
                if hasattr(strategy, "traderData"):
                    traderData = strategy.traderData
